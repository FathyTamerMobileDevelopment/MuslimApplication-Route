...
class RadioFragment : Fragment() {
    ...
    private fun onNextClick() {
        binding.ivNext.setOnClickListener {
            currentRadioIndex = if (currentRadioIndex == radioList.size - 1) 0 else ++currentRadioIndex
    ...
    private fun onPreviousClick() {
        binding.ivPrevious.setOnClickListener {
            currentRadioIndex = if (currentRadioIndex == 0) radioList.size - 1 else --currentRadioIndex
            radioService?.mediaPlayer?.reset()
            radioService?.initMediaPlayer(
                radioList[currentRadioIndex].url, radioList[currentRadioIndex].name
            )
    ...
    private fun getRadiosList() {
        radioList = listOf(
            ...
            ModelRadio("إذاعة محمود علي البنا", "https://backup.qurango.net/radio/mahmoud_ali__albanna_mojawwad"),
            ModelRadio("إذاعة محمود خليل الحصري", "https://backup.qurango.net/radio/mahmoud_khalil_alhussary_warsh"),
            ModelRadio("الإذاعة العامة - اذاعة متنوعة لمختلف القراء", "https://backup.qurango.net/radio/mix"),
            ModelRadio("المختصر في تفسير القرآن الكريم", "https://backup.qurango.net/radio/mukhtasartafsir")
        )
    }
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            if (radioService?.getIsPlaying() == true) {
                binding.ivPlay.setImageResource(R.drawable.ic_pause)
            } else {
                binding.ivPlay.setImageResource(R.drawable.ic_play)
            }
